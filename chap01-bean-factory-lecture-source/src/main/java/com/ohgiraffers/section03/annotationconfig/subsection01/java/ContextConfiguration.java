 ...
 *  beand로 등록하는 방법을 다룬다.(실제 bean으로 등록할 때 자주 쓰는 어노테이션)
 * */
@Configuration

// 기본적으로 같은 경로만 탐색, 다른 경로를 보게 하려면 추가 조치 필요
/* 설명. 1. 기본적으로는 설정 파일이 있는 패키지 및 하위만 scan하지만 basePackages를 바꾸면 다른 범위까지 가능하다. */
...
@ComponentScan(basePackages = {"com.ohgiraffers", "com"},
        ...
        includeFilters = {
                @ComponentScan.Filter(
                        type = FilterType.ASSIGNABLE_TYPE,
                        value = {MemberDAO.class}               // MemberDAO 타입 bean만 등록
                )
        }
)
